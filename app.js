...
form.addEventListener("submit", function (e) {
  ...
  if (isNaN(value) || value <= 0 || value > text.length - 1) {
    ...
    console.log("for values of less than 1 and more than 1 it will return only one paragraph")
  } else {
    let slices = text.slice(0, value);
    slices = slices.map(function (para) {
      return `<p class="result"> ${para}</p>`;
    }).join("");
    result.innerHTML = slices;
  }
